...
这份文档只针对通常意义下的双向链表的实现，要实现`FreeRTOS`中的`list.c`，只需要改变几个数据域即可。
...
+ 在尝试编写 `FreeRTOS`中`list.c`的过程中，我们发现`List`和`ListItem`之间似乎存在着相互的引用，这意味着两者的生命周期必须一样，这似乎有些矛盾，因为`List`的生命周期显然要比`ListItem`的生命周期长。我们的初步想法是重新构建`ListItem`中的某些`成员`，让其不直接指向`List`。 
+ 在编写`rust`的程序时，最为头疼的可能就是其变量的**生命周期**了。暂时接触的比较少，需要进一步加强练习。
+ 尽管我们已经仔细阅读过`官方文档`，但是仍然对其`标准库`不太熟悉，时常会有`invent the wheel`的行为，这增加了我们的负担。
