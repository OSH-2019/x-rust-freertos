...
macro_rules! set_scheduler_suspended {
    ($next_val: expr) => (
        unsafe {
            crate::task::SCHEDULER_SUSPENDED = $next_val
        }
    )
}
...
macro_rules! get_scheduler_suspended {
    () => (
        unsafe {
            crate::task::SCHEDULER_SUSPENDED
        }
    )
}
...
macro_rules! set_top_ready_priority {
    ($new_top_ready_priority: expr) => (
        unsafe {
            crate::task::TOP_READY_PRIORITY = $new_top_ready_priority;
        }
    )
}
...
macro_rules! set_pended_ticks {
    ($next_val: expr) => (
        unsafe {
            crate::task::PENDED_TICKS = $next_val
        }
    )
}
...
macro_rules! set_task_number {
    ($next_val: expr) => (
        unsafe {
            crate::task::TASK_NUMBER = $next_val
        }
    )
}
...
macro_rules! set_yield_pending {
    ($true_or_flase: expr) => (
        unsafe {
            crate::task::YIELD_PENDING = $true_or_flase;
        }
    )
}
...
macro_rules! set_current_number_of_tasks {
    ($next_val: expr) => (
        unsafe {
            crate::task::CURRENT_NUMBER_OF_TASKS = $next_val
        }
    )
}
...
macro_rules! get_current_number_of_tasks {
    () => (
        unsafe {
            crate::task::CURRENT_NUMBER_OF_TASKS
        }
    )
}
...
macro_rules! set_scheduler_running {
    ($true_or_flase: expr) => (
        unsafe {
            crate::task::SCHEDULER_RUNNING = $true_or_flase
        }
    )
}
...
macro_rules! get_next_task_unblock_time {
    () => (
        unsafe {
            crate::task::NEXT_TASK_UNBLOCK_TIME
        }
    )
}
...
macro_rules! set_next_task_unblock_time {
    ($new_time: expr) => (
        unsafe {
            crate::task::NEXT_TASK_UNBLOCK_TIME = $new_time;
        }
    )
}
...
macro_rules! set_tick_count {
    ($next_tick_count: expr) => (
        unsafe {
            crate::task::TICK_COUNT = $next_tick_count;
        }
    )
}
