...
type List = Vec<Arc<RwLock<ListItem>>>;
...
pub struct ListItem {
    ...
    owner: Option<Arc<RwLock<TCB>>>,      // the TCB declaration is not defined
}
...
macro_rules! get_owner_of_next_entry {
    ($list:expr, $item:expr) => ({
        ...
        match index {
            Some(index) => {
                match $list[(index + 1) % current_list_length!($list)].read().unwrap().owner {
...
macro_rules! get_item_index {
    ($list:expr, $item:expr, eq) => ({
        {
            let index = $list.iter().position(|x| x.read().unwrap().item_value == $item.read().unwrap().item_value);
            index
        }
    });
    ...
    ($list:expr, $item:expr) => ({
        {
            ...
            match index {
                Some(index) => $list.insert(index, Arc::clone(&$item)),
    ...
    ($list:expr, $item:expr) => ({
        {
            ...
            match index {
                Some(index) => {
                    Arc::clone(&$list[(index + 1) % current_list_length!($list)])
                },
...
mod test {
    ...
    fn test_basic() {
        ...
        assert_eq!(list1[0].read().unwrap().item_value, 100);
    }
    #[test]
    fn test_some_macros() {
        ...
        assert_eq!(list1[0].read().unwrap().item_value, 100);
        set_list_item_container!(item1, ListName::LIST0);
        assert_eq!(list1[1].read().unwrap().item_value, 200);
        set_list_item_container!(item1, ListName::LIST0);
        assert_eq!(list1[2].read().unwrap().item_value, 300);
        set_list_item_container!(item1, ListName::LIST0);
        assert_eq!(list1[3].read().unwrap().item_value, 400);

        list_remove!(list1, item3);
        assert_eq!(list1[0].read().unwrap().item_value, 100);
        assert_eq!(list1[1].read().unwrap().item_value, 200);
        assert_eq!(list1[2].read().unwrap().item_value, 400);
